...
write.csv(table_save.fq, file = "../../Outputs/tables/ParticipantChar/Characteristics_Female_byTMAOQuantile_211203.csv")

table_save.mq <- print(table_tmao.mq, quote = FALSE, noSpaces = TRUE, printToggle = FALSE)
write.csv(table_save.mq, file = "../../Outputs/tables/ParticipantChar/Characteristics_Male_byTMAOQuantile_211203.csv")

table_save.allq <- print(table_tmao.allq, quote = FALSE, noSpaces = TRUE, printToggle = FALSE)
write.csv(table_save.allq, file = "../../Outputs/tables/ParticipantChar/Characteristics_All_byTMAOQuantile_211203.csv")
```

Make similar tables but for TMAO tertile 
...
Load the Dietary Inflammation Index Score

```{r Dietary Inflammation Index}
# Load the diet scores from Levi and grep for the dietary inflammation index scores only
inflm <- read.csv("../../data/raw/from_LeviEvans/HEI scores_for LE_6142021.csv", row.names = 1)
inflm <- inflm[,grep("dii", colnames(inflm))]

# Combine with the full data from above
full <- merge(full, inflm, by = 0)
row.names(full) <- full[,"Row.names"]
full <- subset(full, select = -c(Row.names))
```
...
Check the relationship between the dietary inflammation index and fasting plasma TMAO. 

```{r TMAO ~ dii - LCMS BD1 - lm 356 ppl, results='hide'}
transformed_keep_tbl2 <- c("sex", 
#"sex_numeric",
"kcal_tnfs", 
"age", 
"cystatinc_bd1", 
...
# Make a smaller df from master (the old data)
full_sub <- full[, colnames(full) %in% transformed_keep_tbl2]
dii <- subset(full_sub, select = -c(sex, kcal_tnfs, age, cystatinc_bd1, ethnicity, tmao, tmao_log))
n <- ncol(dii)

# LCMS method
# Run linear model
my_lm <- lapply(1:n, function(x) lm(tmao_log ~ dii[,x] + sex*age + cystatinc_bd1 + kcal_tnfs, full_sub))
...
my_lm356_lcms <- cbind(my_lm2, my_lm2.5, my_lm2.6, my_lm2.7, my_lm3)
colnames(my_lm356_lcms) <- c("adj.r.squared", "estimate", "Std.Error", "T.value","P")
row.names(my_lm356_lcms) <- colnames(dii)
res_dii <- as.data.frame(my_lm356_lcms)

# BH correction
res_dii$p.adj <- stats::p.adjust(res_dii$P, method = "BH", n = n) 
```

There are no significant relationships after adjusting for multiple corrections in the full cohort. Prior to multiple testing correction, there is a significant finding with Vitamin C but that is not clearly supported by the literature.

Save for your records.

```{r save food linear regression table}
# Save table
write.csv(res_dii, "../../Outputs/tables/Diet/MultipleLinReg_TMAO_LCMS_n356_DII.csv")
```

Double check by sex

```{r TMAO ~ dii - LCMS BD1 male and female - lm 356 ppl, results='hide'}
transformed_keep_tbl2 <- c("sex", 
#"sex_numeric",
"kcal_tnfs", 
"age", 
...
#~~~~~~~~~~~~~
# Make a smaller df from master (the old data)
full_sub_F <- full[, colnames(full) %in% transformed_keep_tbl2]
full_sub_F <- full_sub_F[full_sub_F$sex == "Female",]
dii_F <- subset(full_sub_F, select = -c(sex, kcal_tnfs, age, cystatinc_bd1, ethnicity, tmao, tmao_log))
n <- ncol(dii_F)
# LCMS method
# Run linear model
my_lm <- lapply(1:n, function(x) lm(tmao_log ~ dii_F[,x] + age + cystatinc_bd1 + kcal_tnfs, full_sub_F ))
...
my_lm356_lcms <- cbind(my_lm2, my_lm2.5, my_lm2.6, my_lm2.7, my_lm3)
colnames(my_lm356_lcms) <- c("adj.r.squared", "estimate", "Std.Error", "T.value","P")
row.names(my_lm356_lcms) <- colnames(dii_F)
res_dii_F <- as.data.frame(my_lm356_lcms)

# BH correction
res_dii_F$p.adj <- stats::p.adjust(res_dii_F$P, method = "BH", n = n) 

#~~~~~~~~~~~~~
# MALES 
#~~~~~~~~~~~~~
# Make a smaller df from master (the old data)
full_sub_M <- full[, colnames(full) %in% transformed_keep_tbl2]
full_sub_M <- full_sub_M[full_sub_M$sex == "Male",]
dii_M <- subset(full_sub_M, select = -c(sex, kcal_tnfs, age, cystatinc_bd1, ethnicity, tmao, tmao_log))
n <- ncol(dii_M)
# LCMS method
# Run linear model
my_lm <- lapply(1:n, function(x) lm(tmao_log ~ dii_M[,x] + age + cystatinc_bd1 + kcal_tnfs, full_sub_M ))
...
my_lm356_lcms <- cbind(my_lm2, my_lm2.5, my_lm2.6, my_lm2.7, my_lm3)
colnames(my_lm356_lcms) <- c("adj.r.squared", "estimate", "Std.Error", "T.value","P")
row.names(my_lm356_lcms) <- colnames(dii_M)
res_dii_M <- as.data.frame(my_lm356_lcms)

# BH correction
res_dii_M$p.adj <- stats::p.adjust(res_dii_M$P, method = "BH", n = n) 
```

Still no significant results after BH correction. Prior to BH correction, there is a suggested relationship with folic acid and TMAO in males. 
...
write.csv(table_save.fq, file = "../../Outputs/tables/ParticipantChar/Food_Female_byTMAOQuantile.csv")

table_save.mq <- print(table_food.mq, quote = FALSE, noSpaces = TRUE, printToggle = FALSE)
write.csv(table_save.mq, file = "../../Outputs/tables/ParticipantChar/Food_Male_byTMAOQuantile.csv")

table_save.allq <- print(table_food.allq, quote = FALSE, noSpaces = TRUE, printToggle = FALSE)
write.csv(table_save.allq, file = "../../Outputs/tables/ParticipantChar/Food_All_byTMAOQuantile.csv")
```
...
"waistavg_cm", "sysbpavgv2", "diabpavgv2", "endo_rhi", "endo_al75", "hdl_bd1", "ldl_bd1", "chol_bd1", "tg_bd1_ln", "glc_bd1_ln","tnfa_bd1_ln", "il6_bd1_ln", "crp_bd1_ln",
...
summary(lm(glc_bd1_ln ~  tmao_log + sex*age + cystatinc_bd1, full[full$low_mps_meat == "high",]))
summary(lm(glc_bd1_ln ~  tmao_log + sex*age + cystatinc_bd1, full[full$low_mps_meat == "high",]))
```
