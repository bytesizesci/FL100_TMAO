...
1) Plots of differential abundance, and alpha and beta diversity results.
2) Tables used in the supplemental files of the manuscript.
...
**Updates:**
On 01-19-2022 I reran many of the analyses with the upadted phyloseq object PSOtmao_NA to see if my results were consistent. I saved this markdown to the update-taxonomy-table branch of the GitHub FL100_TMAO project. Background - When MK was reading my manuscript, she noticed that I have some entries in the taxonomy table that say NA and some that say "g__" or "f__". While these may have different meanings, she suggests simply changing all cases to NA. (Another solution, you could manually differentaite "g__" to "g__ASV1" so that the glomming isn't wrong). 
...
```{r Save the taxonomy table for manuscript}
...
Table of read counts at the Phylum level. We will filter low abundant Phyla. We will also require that taxa are present in at least 5% of samples. We will then agglomerate at the Genus level. 
...
# Save
tiff("../../Outputs/plots/microbiome/RelAbund_Fam_TMAOTertiles_2201.tiff", width = 5, height = 4, units = 'in', res = 300)
plot.composition.relAbun
dev.off()
```
...
# Loop Plot to view
nres <- length(resOTU)
comparisons <- list(c("Tertile1", "Tertile2"), c("Tertile1", "Tertile3"), c("Tertile2","Tertile3"))
for (i in 1:nres) { 
    print(ggplot(ggData_G, aes(x = tmao_tertile, y = log(ggData_G[,i]))) +
            geom_boxplot(aes(color=tmao_tertile)) +
            geom_point() +
            theme_minimal() +
            ylab(paste0("Log of", colnames(ggData_G[i]))) +
            xlab("TMAO Tertile") +
            scale_color_brewer(palette = "Dark2") +
            labs(color = "TMAO Tertile") +
...
comparisons <- list(c("Tertile1", "Tertile2"), c("Tertile1", "Tertile3"), c("Tertile2","Tertile3"))
for (i in 1:nres) { 
    temp_plot <- ggplot(ggData_G, aes(x = tmao_tertile, y = log(ggData_G[,i]))) +
            geom_boxplot(aes(color=tmao_tertile)) +
            geom_point() +
            theme_minimal() +
            ylab(paste0("Log of \n", colnames(ggData_G[i]))) +
            ...
            labs(color = "TMAO Tertile") 
            #stat_compare_means(comparisons=comparisons)
    tiff(paste0("../../Outputs/plots/microbiome/DESeq2_Genus_TMAOTertile_Boxplot_", i,"_2201.tiff"), width = 4, height = 4, units = 'in', res = 300)
    print(temp_plot)
    dev.off()
}
...
```{r Load bdiversity calculations}
...
tiff("../../Outputs/plots/microbiome/BetaDiversity_PCoA_WU_TMAOmdn_2201.tiff", width = 6, height = 4, units = 'in', res = 300)
betaplot_WU
dev.off()

# unweighted
#tiff("../../Outputs/plots/microbiome/BetaDiversity_PCoAWU_TMAOmdn_2112.tiff", width = 6, height = 4, units = 'in', res = 300)
tiff("../../Outputs/plots/microbiome/BetaDiversity_PCoA_UWU_TMAOmdn_2201.tiff", width = 6, height = 4, units = 'in', res = 300)
betaplot_UWU
dev.off()
```
...
```{r Save Genus tertile prevalence csv}
...
Repeat TMAO analyses at the Family Level
...
# Now subset at the genus level to identify the most common genus
# Subset by tertiles (Genus)
PSOtmao1F_tertile1 <- subset_samples(PSOtmao1F, tmao_tertile == "Tertile1")
PSOtmao1F_tertile2 <- subset_samples(PSOtmao1F, tmao_tertile == "Tertile2")
PSOtmao1F_tertile3 <- subset_samples(PSOtmao1F, tmao_tertile == "Tertile3")

#~~~~~~~~~~
# Tertile 1
#~~~~~~~~~~
# Prevalence - how many samples the taxa is observed in
prevalenceF_tert1 = apply(X = otu_table(PSOtmao1F_tertile1),
               MARGIN = ifelse(taxa_are_rows(PSOtmao1F_tertile1), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})

prevalenceF_tert1 = data.frame(Prevalence = prevalenceF_tert1,
                    TotalAbundance = taxa_sums(PSOtmao1F_tertile1), # the total number of occurrences (even if only observed 50 times in 1 sample, the number would be 50)
                    tax_table(PSOtmao1F_tertile1))
...
#~~~~~~~~~~
# Prevalence - how many samples the taxa is observed in
prevalenceF_tert2 = apply(X = otu_table(PSOtmao1F_tertile2),
               MARGIN = ifelse(taxa_are_rows(PSOtmao1F_tertile2), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})

prevalenceF_tert2 = data.frame(Prevalence = prevalenceF_tert2,
                    TotalAbundance = taxa_sums(PSOtmao1F_tertile2), # the total number of occurrences (even if only observed 50 times in 1 sample, the number would be 50)
                    tax_table(PSOtmao1F_tertile2))
#~~~~~~~~~~
# Tertile 3
#~~~~~~~~~~
# Prevalence - how many samples the taxa is observed in
prevalenceF_tert3 = apply(X = otu_table(PSOtmao1F_tertile3),
               MARGIN = ifelse(taxa_are_rows(PSOtmao1F_tertile3), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})

prevalenceF_tert3 = data.frame(Prevalence = prevalenceF_tert3,
                    TotalAbundance = taxa_sums(PSOtmao1F_tertile3), # the total number of occurrences (even if only observed 50 times in 1 sample, the number would be 50)
                    tax_table(PSOtmao1F_tertile3))
```

```{r Save Family tertile prevalence csv}
write.csv(prevalenceF_tert1, "../../Outputs/tables/Microbiome/Prevalence_Family_Tertile1_2201.csv")
write.csv(prevalenceF_tert2, "../../Outputs/tables/Microbiome/Prevalence_Family_Tertile2_2201.csv")
write.csv(prevalenceF_tert3, "../../Outputs/tables/Microbiome/Prevalence_Family_Tertile3_2201.csv")
```

```{r DESeq2 analysis Family - TMAO tertile}
# DESeq analysis - tmao_tertile
mb <- phyloseq_to_deseq2(PSOtmao1F, ~ sex + age + bmi_final.x + tmao_tertile)
mb.ratio <- DESeq(mb, test = "LRT", reduced = ~ sex + age + bmi_final.x) # remove variable of interest here, TMAO
res.ratio <- results(mb.ratio, contrast = c("tmao_tertile", "Tertile1", "Tertile3")) # Look at most different tertiles
alpha = 0.05
sigtab = res.ratio[which(res.ratio$padj < alpha), ]
sigtab = cbind(as(sigtab, "data.frame"), as(tax_table(PSOtmao1F)[rownames(sigtab), ], "matrix"))
sigtab$order_family <- paste0(sigtab$Order, sigtab$Family)
head(sigtab)
...
#~~~~~~~~~~~~~~~
resOTU <- rownames(sigtab)
otuData <- as.data.frame(otu_table(PSOtmao1F))
otuData_desRes <- otuData[rownames(otuData) %in% resOTU,]
#otuData_desRes <- as.data.frame(t(otuData_desRes))
# Get sample data groupings
sampleData <- as.data.frame(sample_data(PSOtmao1F))
sampleData <- subset(sampleData, select = c(tmao_mdn, tmao_quantile, tmao_tertile, tmao))
# Get tax table
taxaData <- as.data.frame(tax_table(PSOtmao1F))
taxaData_res <- taxaData[rownames(taxaData) %in% resOTU,]
taxaData_res$OrderFamily <- paste0(taxaData_res$Order, taxaData_res$Family)

# Merge
ggData <- merge(taxaData_res, otuData_desRes, by = 0)
rownames(ggData) <-  ggData$OrderFamily
ggData <- ggData[,-(1:9)]
ggData <- as.data.frame(t(ggData))
ggData <- merge(ggData, sampleData, by = 0)
rownames(ggData) <- ggData[,"Row.names"]
...
nres <- length(resOTU)
for (i in 1:nres) { 
    print(ggplot(ggData_F, aes(x = tmao_tertile, y = log(ggData_F[,i]))) +
            geom_boxplot(aes(color=tmao_tertile)) +
            geom_point() +
            theme_minimal() +
            ylab(paste0("Log of", colnames(ggData_F[i]))) +
            xlab("TMAO Tertile") +
            scale_color_brewer(palette = "Dark2") +
            labs(color = "TMAO Tertile")
    )
...
nres <- length(resOTU)
for (i in 1:nres) { 
    temp_plot <- ggplot(ggData_F, aes(x = tmao_tertile, y = log(ggData_F[,i]))) +
            geom_boxplot(aes(color=tmao_tertile)) +
            geom_point() +
            theme_minimal() +
            ylab(paste0("Log of", colnames(ggData_F[i]))) +
            ...
            scale_color_brewer(palette = "Dark2") +
            labs(color = "TMAO Tertile")
    tiff(paste0("../../Outputs/plots/microbiome/DESeq2_Family_TMAOTertile_Boxplot_", i,"_2201.tiff"), width = 4, height = 4, units = 'in', res = 300)
    print(temp_plot)
    dev.off()
}
...
DESeqPlot <- ggplot(sigtabgen, aes(y=order_family, x=log2FoldChange, color=Phylum)) + 
  geom_vline(xintercept = 0.0, color = "gray", size = 0.5) +
  geom_point(size=3) + 
  ylab("Taxonomy") +
  xlab("Log2 Fold Change") + 
...
# Save
tiff("../../Outputs/plots/microbiome/DESeq2_Family_TMAOTertile1v3_2201.tiff", width = 8, height = 3, units = 'in', res = 300)
DESeqPlot
dev.off()
```
Save

```{r save deseq2 Family tmao tertile results}
#write.csv(sigtabgen, "../../Outputs/tables/Microbiome/DESeq2_PSOtmao1G_TMAOTertile_2112.csv")
write.csv(sigtabgen, "../../Outputs/tables/Microbiome/DESeq2_PSOtmao1F_TMAOTertile_220120.csv")
```
...
```{r Get the genus microbiome data ready to combine with pheno data}
...
New results:
f__Erysipelotrichaceae_g__Catenibacterium + f__Erysipelotrichaceae_g__Coprobacillus + f__Lachnospiraceae_g__Roseburia + f__Lachnospiraceae_g__Butyrivibrio
...
```{r Save linear regression results with bacteria}
...
Is TNF-a related to the gut microbiome directly?
## Factored by tnfa Tertile 


```{r DESeq2 analysis - tnf-a tertile}
# Quick check
...
summary(lm(tnfa_bd1_ln ~ f__Erysipelotrichaceae_g__Coprobacillus + sex + age + bmi_final, phen_sub)) # NS
...
```{r Christensenellaceae family and BMI}
# Note = ggData is the family level DESeq results 
fam_res <- merge(ggData, phen, by = 0)
rownames(fam_res) <- fam_res[,"Row.names"]
fam_res <- fam_res[,-1]
colnames(fam_res) <- gsub(" ", "_", colnames(fam_res))

summary(lm(`_o__Clostridiales_f__Christensenellaceae` ~ bmi_final + sex + age, fam_res))
```